...
class InstrumentedPCA(BaseEstimator):
    ...
    def __init__(self, n_factors=1, intercept=False, max_iter=10000,
        ...
        print("the maxiter are", max_iter)
...
#         print(self.max_iter)
    ...
    def _fit_ipca(self, X=None, y=None, indices=None, PSF=None, Q=None,
        ...
        while((iter <= self.max_iter) and (tol_current > self.iter_tol)):
            ...
            iter += 1
            if not quiet:
                print('Step', iter, '- Aggregate Update:', tol_current, "- my itertol is:", self.iter_tol,
                     "- my maxiter is:", self.max_iter)
            if (iter > self.max_iter) or (tol_current <= self.iter_tol):
                print('stop', iter, '- Aggregate Update:', tol_current, "- my itertol is:", self.iter_tol,
                     "- my maxiter is:", self.max_iter)
...
def _numba_solve(m1, m2):
    return np.linalg.lstsq(np.ascontiguousarray(m1), np.ascontiguousarray(m2))[0]
